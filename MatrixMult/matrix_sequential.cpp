...
void errorExit(const char* progname, const char* error) {
    ...
    std::cerr << "usage: " << progname << " <matrix1> <matrix2> <num_threads>" << std::endl
              << "\twhere <matrix1> and <matrix2> are file names containing matrices,"
              << "\tand <num_threads> is the desired number of threads." << std::endl;
    exit(EXIT_FAILURE);
}
...
int main(int argc, char** argv) {
    ...
    if (num_threads <= 0) {
        errorExit(argv[0], "Invalid number of threads.");
    }
